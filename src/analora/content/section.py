...
from analora.utils.html import GO_TO_TOP, render_toc, tags2id, tags2title, valid_h_tag
...
def create_template() -> str:
    ...
    >>> from analora.content.section import create_template
    >>> template = create_template()

    ```
    """
    return """<h{{depth}} id="{{id}}">{{section}} {{title}}
           </h{{depth}}> {{go_to_top}} <p style="margin-top: 1rem;">
           {{content}} <p style="margin-top: 1rem;">"""
